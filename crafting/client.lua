...
function CraftItem(cate_id, item_id, amount)
	...
	or Config["craft_cost"] ~= nil and Config["craft_cost"] > 0 then
		...
		if type(cost) ~= "number" then
			SendNUIMessage({
				type = 'dialog',
				title = "Erro de arquivo de configuração",
				message = "Contate o administrador do sistema para corrigir o problema."
			})
			return
		end
			...
			for k,v in ipairs(Config["craft_table"]) do
				if v.position ~= nil then
					if v.ent == nil then
						if ESX then
							...
							if v.map_blip then
								...
								AddTextComponentString(v.blip_name or "Mesa de Trabalho")
					...
					if v.ent and type(v.ent) == "number" and craft_percent ~= -1 then
						draw.Simple3DText(v.position["x"], v.position["y"], v.position["z"] + 1.4, "Construindo", 1.0)
					...
					if dist <= (v.max_distance or 2.0) and craft_percent == -1 and not menu_active then
						...
						AddTextComponentString("Clique ~INPUT_CONTEXT~ para aceder á Mesa de Trabalho")
